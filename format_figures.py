...
def format_save_plot(title="", show=True):
    format_plot()
    if title:
        plt.savefig(join(pardir, "figures", title + '.jpg'))
    if show:
        plt.show()
...
plt.rcParams['axes.titlepad'] = 6.0  # pad between axes and title in points
plt.rcParams['axes.labelsize'] = 24  # fontsize of the x any y labels

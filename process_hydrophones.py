...
from common_nipissis import root_dir, sensitivity_h, rms
...
#        print('Processing '+f)
    ...
    if len(starttime) > 0 :
        ...
        plt.grid()
        plt.title('Hydrophones 2019-08-0'+str(j), fontsize=18)
        plt.ylabel('24 channel mean RMS amplitude (Pa)', fontsize=14)
        plt.xlabel('Time', fontsize=14)
        plt.xlim(datetime.datetime(2019,8,j-1,23,50), datetime.datetime(2019,8,j+1,0, 10))
        plt.tight_layout()
        plt.savefig('fig/h_2019-08-0'+str(j)+'_mean_rms.pdf', bbox_inches='tight')
